# Firefox Stable — for WebAPI research
#
# Build:
#   docker build -f examples/firefox-stable.Dockerfile -t firefox-stable .
#
# Verify:
#   docker run --rm firefox-stable --version
#
# Run PoC:
#   docker run --rm --shm-size=2g \
#     -v /path/to/poc.html:/tmp/poc.html:ro \
#     firefox-stable \
#     --screenshot /tmp/out.png file:///tmp/poc.html
...
# ── Install Firefox Stable from Mozilla APT repo ─────────────────────────
...
# Smoke test
RUN firefox --version 2>/dev/null || true

ENTRYPOINT ["firefox", "-profile", "/tmp/ff-profile", "--headless", "--no-remote"]
